...
applyFilter(struct Filter *filter, cs1300bmp *input, cs1300bmp *output)
{
  ...
  int *temp;
  int *temp1;																//declaring temps which represent splitting up matrix into 3 parts 
  int *temp2;
   ...
   filter->get(2, 2)};
	...
	for(int col = 1; col < input_width; col++) {
		for(int row = 1; row < input_height; row++) {
		  ...
		  value2 = 0;
		  value3 = 0;
			temp = &tempFil[0];										//extracting next 3 values in array by column
			temp1 = temp + 1;
			temp2 = temp + 2;
			...
			color2 = &input -> color[0][col][row1];			//ordered by plane, col, row
			color3 = &input -> color[0][col3][row1];
			
		  value = value +  *color1 * (*temp);
		  value2 = value2 +  *(color1 + 1) * (*temp1);	//matrix multiplication
		  value3 = value3 +  *(color1 + 2) * (*temp2);
		  temp += 3;
		  temp1 += 3;																	//shifting to next row of tempFil array
		  temp2 += 3;
		  value = value +  *color2 * (*temp);
		  value2 = value2 +  *(color2 + 1) * (*temp1);
		  value3 = value3 +  *(color2 + 2) * (*temp2);
		  temp += 3;
		  temp1 += 3;																	//shift
		  temp2 += 3;
		  value = value +  *color3 * (*temp);
		  value2 = value2 +  *(color3 + 1) * (*temp1);
		  value3 = value3 +  *(color3 + 2) * (*temp2);
		  value += value2 + value3;
		  value = value / divisor;
		  if (value  < 0) { value = 0; }
		  ...
		  value = 0;																//repeats 3x for each color, only thing changing is which plane
		  value2 = 0;
		  value3 = 0;
			temp = &tempFil[0];
			temp1 = temp + 1;
			...
			color1 = &input -> color[1][col1][row1];
			color2 = &input -> color[1][col1][row];
			color3 = &input -> color[1][col3][row1];
			
		  value = value +  *color1 * (*temp);
		  value2 = value2 +  *(color1 + 1) * (*temp1);
		  value3 = value3 +  *(color1 + 2) * (*temp2);
		  temp += 3;
		  temp1 += 3;
		  temp2 += 3;
		  ...
		  value3 = 0;
			...
			color1 = &input -> color[2][col1][row1];
			color2 = &input -> color[2][col][row1];
			color3 = &input -> color[2][col3][row1];
